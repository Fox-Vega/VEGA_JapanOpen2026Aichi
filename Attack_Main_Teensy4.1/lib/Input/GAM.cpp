...
constexpr int16_t kAccelOffsetX = 2;
...
constexpr int16_t kAccelRadius = 0;
...
float GAM::get_atob(float azimuth) {
...
float GAM::get_btoa(float bimuth) {
...
void GAM::cal_ctob(float x, float y) {
    float result = myvector.get_azimuth(x, y) + get_azimuth();
    while (result < 0) result += 360.0f;
    while (result >= 360.0f) result -= 360.0f;
    myvector.cal_cord(result, myvector.get_magnitude(x, y));
    x = myvector.get_x();
    y = myvector.get_y();
}

void GAM::cal_btoc(float x, float y) {
    float result = myvector.get_azimuth(x, y) - get_azimuth();
    while (result < 0) result += 360.0f;
    while (result >= 360.0f) result -= 360.0f;
    myvector.cal_cord(result, myvector.get_magnitude(x, y));
